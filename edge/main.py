...
검사 파이프라인 흐름:
    ┌──────────────────────────────────────────────────────────────┐
    │  1. 카메라 캡처 (1080p)                                        │
    │  2-A. Stage 1: YOLO → 피듀셜 마크 탐지 → 정렬 각도 계산        │
    │  2-B. Stage 2: 정렬된 ROI Crop → YOLO → 결함 탐지              │
    │  3. 판정 (PASS / FAIL)                                         │
    │  4. GPIO 즉시 알람 (부저 + LED)                                 │
...
async def run_inspection_pipeline() -> Optional[InspectionPacket]:
    ...
    try:
        ...
        logger.info(
            "[파이프라인] 정렬 결과: %s, 오차: %.2f°",
            "정렬 OK" if alignment.is_aligned else "정렬 오류",
            alignment.angle_error_deg,
        )
        ...
        # 정렬 실패 시 즉시 FAIL 판정 (Stage 2 건너뜀)
        if not alignment.is_aligned:
            logger.warning("[파이프라인] 정렬 오류 → FAIL 판정, Stage 2 건너뜀")
